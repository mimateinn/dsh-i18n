...
- **자동 번역**: 비중국어 locale이 활성화되면 긴 영어 텍스트(플러그인 마켓 설명, 타사 UI, 오류 문구)가 설정된 모델을 통해 자동으로 사용자의 언어로 번역됩니다. 캐시되고 멱등(idempotent)하여 React 재렌더링과 충돌하지 않습니다. 기본 언어(en/zh)는 그대로 두며, 번체 중국어는 모델을 호출하는 대신 내장된 간체→번체 변환을 계속 사용합니다.
...
- 자동 번역은 타사 API가 아닌 DSH 내장 LLM 서비스(설정된 모델)를 통해 실행됩니다. 비중국어 locale이 활성화된 경우에만 긴 영어 산문에 대해 작동하며, 기본 언어는 번역을 위해 전송되지 않습니다.
