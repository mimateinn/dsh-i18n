...
- **沉浸式翻譯**：當啟動嘅係非中文語言時，長英文文字（插件市場描述、第三方 UI、錯誤訊息）會經你用緊嘅模型自動翻譯成你嘅語言——有快取同冪等，React 重新渲染唔會反覆覆寫。預設語言（en／zh）唔翻譯；繁體中文繼續用內置簡→繁轉換，唔使叫模型。
...
- 沉浸式翻譯經 DSH 內置 LLM 服務（你用緊嘅模型）執行，唔經第三方 API。只喺非中文語言啟動時、對長英文散文觸發；預設語言永遠唔會送出去翻譯。
