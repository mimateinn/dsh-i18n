...
- **Traduction automatique** : lorsqu'une locale non chinoise est active, les longs textes en anglais (descriptions du marché des plugins, UI tierce, prose d'erreur) sont traduits automatiquement dans votre langue via votre modèle configuré — mis en cache et idempotent, de sorte que les re-rendus de React n'entrent pas en conflit. La langue par défaut (en/zh) n'est pas modifiée ; le chinois traditionnel conserve la conversion simplifié → traditionnel intégrée au lieu d'appeler un modèle.
...
- La traduction automatique passe par le service LLM intégré de DSH (votre modèle configuré), et non par une API tierce. Elle ne se déclenche que pour de longs textes en anglais lorsqu'une locale non chinoise est active ; la langue par défaut n'est jamais envoyée à la traduction.
